...
class TrainFaces:
    ...
    def capture_training_images(self):
        video_capture = cv2.VideoCapture('rtsp://192.168.43.1:8080/h264_pcm.sdp')        # video_capture = cv2.VideoCapture(0)
        while True:
            self.count_timer += 1
...
class RecogFaces:
    ...
    def show_video(self):
        video_capture = cv2.VideoCapture('rtsp://192.168.43.1:8080/h264_pcm.sdp')
    ...
    def process_image(self, inImg):
        ...
        for i in range(len(faces)):
            ...
            if confidence[1] < 100:
                ...
                # cv2.putText(frame, '%s - %.0f' % (person, confidence[1]), (x - 10, y - 10), cv2.FONT_HERSHEY_PLAIN, 1,
                #             (0, 255, 0))

                # put a text box under the face
                cv2.rectangle(frame, (x, y + h), (x + w, y + h + 30), (255, 0, 0), cv2.FILLED)
                cv2.putText(frame, person, (x + 10, y + h + 25), cv2.FONT_HERSHEY_DUPLEX, 0.7, (255, 255, 255), 1)

            else:
                person = 'Unknown'
                cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 0, 255), 2)
                # cv2.putText(frame, person, (x - 10, y - 10), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0)) # (x-10, y-10) is the position of the text
                # persons.append(person)

                cv2.rectangle(frame, (x, y + h), (x + w, y + h + 30), (0, 0, 255), cv2.FILLED)
                cv2.putText(frame, person, (x + 10, y + h + 25), cv2.FONT_HERSHEY_DUPLEX, 0.7, (255, 255, 255),
                            1)  # 0.5 ís the size of the text
